...
async function generateEmbedding(text) {
    try {
        ...
        // text-embedding-004 defaults to 768 usually, but if it returns 3072 we handle it.
        // However, the SDK might not support outputDimensionality param in all versions cleanly.
        // Let's check the values length. 
        // If 3072, we might need a different approach or model parameter.
        // According to docs, text-embedding-004 output dimension is configurable.
        // But let's try to just slice it if it's Matryoshka compatible, OR 
        // use the correct parameter if SDK supports it.
        // Better: let's try to pass it in the model config if possible, or request config.

        // Retrying with correct simple call first, but checking if we can slice.
        // text-embedding-004 IS Matryoshka-compliant, so we can just slice the first 768 values!
